...
#define QCB_MV_MAX  20000		  // Максимальное напряжение для QC3.0 адаптеров класса B	
...
public:
    ...
    uint8_t begin() {
        ...
        delayMicroseconds(TEST_PULSE);  // Задержка в несколько микросекунд
        if (!digitalRead(_dm_h)) {      // Проверяем (D-), если LOW - (D+) и (D-) внутри адаптера НЕ соеденены
        // Адаптер разъединил (D+) и (D-) после согласования, значит он поддерживает QC2.0 или QC3.0
